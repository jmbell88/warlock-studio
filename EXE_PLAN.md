...
- Manual — **edit in place, no new chapters** (numbering is test-gated): `15-installation.md` installer path first, GGUF section → "downloaded in-app"; `16-configuration.md` note `WARLOCK_TRELLIS_MODELS` also moves the in-app download; `17-app-settings.md` "Reconstruction engine" group + first-run overlay; `18-troubleshooting.md` SmartScreen note + partial-GGUF-reads-missing note. Update INVARIANTS.md record counts ("21 records over 20 repositories").
