# --- VENDORED ---------------------------------------------------------------
#
# ZhengPeng7/BiRefNet's ``birefnet.py``, at commit
# e2bf8e4460fc8fa32bba5ea4d94b3233d367b0e4. See ATTRIBUTION.md beside this file
# for the licence, the upstream source's SHA-256 and the full list of changes.
#
# It is here so the app can stop passing ``trust_remote_code=True``. That flag
# executes whatever Python the repository happens to ship *today*, in the app's
# own process, with the user's filesystem and network permissions -- and
# ``uv.lock`` says nothing at all about code downloaded at runtime (MDL-03).
...
# upstream's style, and reformatting it would destroy the diff that makes it
# auditable.
...
# WARLOCK 1/4: the torchvision imports are gone with the download paths below.
# ``VGG16_Weights.DEFAULT`` and friends are *URLs*: touching one reaches out to
# download.pytorch.org, which this app may never do -- HF_HUB_OFFLINE is set
# before anything else in the package and the only process allowed online is
# the user-initiated fetch child. They were only ever reachable through
# ``pretrained=True``, which the checkpoint's own config sets to False, so the
# whole branch was dead weight pointing at the network.
...
# WARLOCK 2/4: ``pretrained`` is hard-pinned False and the three torchvision
# branches are gone. The checkpoint on disk carries every weight this model
# needs -- ``bb_pretrained: false`` in its own config.json says so -- and the
# only thing ``pretrained=True`` ever did here was fetch ImageNet weights that
# the published state dict then overwrote. The ``eval()`` on the backbone name
# is replaced with an explicit table for the same reason it is replaced
# everywhere else in this file: a name out of a config file reaching ``eval``
# is arbitrary execution with extra steps.
BACKBONES = {}


def _register_backbones():
    """The name -> constructor table, built from what this module defines.

    A function rather than a literal at import time, because the constructors
    are defined *below* this point in upstream's file ordering and moving them
    would be a bigger edit than this one.
    """
    for name in (
        'swin_v1_t', 'swin_v1_s', 'swin_v1_b', 'swin_v1_l',
        'pvt_v2_b0', 'pvt_v2_b1', 'pvt_v2_b2', 'pvt_v2_b5',
    ):
        found = globals().get(name)
        if found is not None:
            BACKBONES[name] = found


def build_backbone(bb_name, pretrained=False, params_settings=''):
    if not BACKBONES:
        _register_backbones()
    if bb_name in ('vgg16', 'vgg16bn', 'resnet50'):
        raise ValueError(
            'the {} backbone needs torchvision pretrained weights, which this '
            'vendored copy deliberately cannot download'.format(bb_name)
        )
    build = BACKBONES.get(bb_name)
    if build is None:
        raise ValueError('unknown BiRefNet backbone: {!r}'.format(bb_name))
    kwargs = {}
    if params_settings:
        # Upstream passed this through ``eval`` as source text. The only value
        # any code path here supplies is ``in_channels=4``, so it is parsed
        # rather than executed.
        for pair in params_settings.split(','):
            key, _, value = pair.partition('=')
            kwargs[key.strip()] = int(value.strip())
    return build(**kwargs)

# Upstream's ``load_weights`` lived here: a bare ``torch.load`` -- no
# ``weights_only`` -- over a path from ``config.weights``. Nothing in this
# build ever called it; the live path loads safetensors. Deleted rather than
# hardened, because the safe version of an unreachable unpickler is still an
# unpickler one import away from being reachable again.
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.config = Config()
        # WARLOCK 3/4 (a): explicit, not ``eval``. These two were already
        # literals, so this is the same objects by a route nobody has to audit.
...
# from models.stem_layer import StemLayer
from .configuration import BiRefNetConfig  # WARLOCK: renamed on vendoring
...
class BiRefNet(
    ...
    def __init__(self, bb_pretrained=True, config=BiRefNetConfig()):
        ...
        if self.config.squeeze_block:
            # WARLOCK 3/4 (b): the block name and the repeat count both came
            # out of the config and both went through ``eval``. The name is a
            # table lookup now and the count is ``int()``.
            _sq_name, _, _sq_times = self.config.squeeze_block.partition('_x')
            self.squeeze_module = nn.Sequential(*[
                BLOCKS[_sq_name](channels[0]+sum(self.config.cxt), channels[0])
                for _ in range(int(_sq_times))
            ])
        ...
        if self.config.refine:
            if self.config.refine == 'itself':
                ...
                # WARLOCK 3/4 (c): a config string was formatted into source
                # and executed. The refiners are a fixed set.
                self.refiner = REFINERS[self.config.refine](in_channels=3+1)
...
class Decoder(nn.Module):
    def __init__(self, channels):
        super(Decoder, self).__init__()
        self.config = Config()
        # WARLOCK 3/4 (d): the last two, and the only ones a config file could
        # actually steer -- ``dec_blk`` and ``lat_blk`` are strings in Config.
        DecoderBlock = BLOCKS[self.config.dec_blk]
        LateralBlock = BLOCKS[self.config.lat_blk]
...
# WARLOCK 3/4 (e): the tables the dispatches above read, in place of ``eval``.
# Built at the end of the module so every class is defined; a name that is not
# in here raises a KeyError, which is the whole point -- an unknown block name
# in a config file must fail loudly rather than execute.
BLOCKS = {
    'BasicDecBlk': BasicDecBlk,
    'BasicLatBlk': BasicLatBlk,
    'ResBlk': ResBlk,
    # ``HierarAttDecBlk`` is named by Config's own list of choices and is not
    # defined anywhere in upstream's file -- selecting it would have been an
    # immediate NameError out of ``eval`` before, and is a KeyError now. Left
    # out rather than stubbed: inventing a class upstream does not have would
    # be the one modification that changes behaviour.
    'ASPP': ASPP,
...
# WARLOCK 4/4: the backbone table, populated now that every constructor exists.
# ``build_backbone`` also builds it lazily; doing it here as well means an
# importer can read ``BACKBONES`` without having built a model, which is what
# the pvt_v2_b0 CPU shape test does -- a real backbone, small enough to
# instantiate in a unit test, exercising the dispatch this file replaced.
_register_backbones()
