...
class ACEStepPipeline:
    ...
    def get_checkpoint_path(self, checkpoint_dir, repo):
        ...
        if checkpoint_dir_models is None:
            ...
            # braces beside `HF_HUB_OFFLINE=1`, exactly as the vendored
            # BiRefNet passes `local_files_only=True`.
            raise FileNotFoundError(
                f"ACE-Step weights are not present in {checkpoint_dir!r} "
                f"(expected the four subfolders of {repo}); "
                "download them from Settings -> Models."
            )
