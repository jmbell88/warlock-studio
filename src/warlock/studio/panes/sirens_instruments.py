...
release point -- are Phase 3 and get a pane of their own, because a sequence
editor is a canvas rather than a row of fields and it does not belong squeezed
into a 300 px sidebar beside the list it edits.
...
the single most confusing thing a tracker can do to a newcomer.
"""
...
def draw(ctx: Any) -> None:
    ...
    imgui.dummy((0, 4))
    for index, instrument in enumerate(list(doc.instruments)):
        label = instrument.name or f"Instrument {index + 1}"
        if controls.selectable(
            f"{index:02X}  {label}  [{instrument.kind}]###sirens-inst-{instrument.uid}",
            state.instrument == instrument.uid,
        )[0]:
            state.instrument = instrument.uid
    ...
    if changed and doc.update_instrument(selected.uid, kind=kind):
        sirens_mode.request_rerender(ctx, tab)
    widgets.muted_wrapped(
        "The envelope editor is not built yet: an instrument uses its kind's"
        " default volume, arpeggio, pitch and duty sequences."
    )
