...
def _filter_control(state: Any, values: dict[str, Any], key: str) -> None:
    ...
    if key in filters.COLOUR_PARAMS:
        ...
        changed, value = imgui.color_edit4(
            key, inker_colors._vec(tuple(values[key])), inker_colors.FLAGS
        )
        if changed:
    ...
    if key in filters.TOGGLE_PARAMS:
        ...
        changed, on = imgui.checkbox(key, bool(values[key]))
        if changed:
    ...
    if choices is not None:
        picked = widgets.combo(
            key, str(values[key]), [(str(choice), str(choice)) for choice in choices]
        )
