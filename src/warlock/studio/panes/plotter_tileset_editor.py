...
* **Animation** -- the frame strip, with a real per-frame ``duration_ms``
  instead of the hard-coded 100 ms the sidebar's "Add frame" wrote.
...
from .. import controls, plotter_mode, theme, widgets
...
def _animation_tab(ctx: Any, state: Any, tab: Any, ref: Any, index: int) -> None:
    """The frame strip, with a duration per frame.

    The sidebar wrote a hard-coded 100 ms and had no way to change it, so every
    animated tile in a map made here ran at exactly ten frames a second.
    """

    local = int(state.editing_tile)
    meta = ref.tileset.meta_of(local)
    widgets.muted(f"Tile {local}: {len(meta.animation)} frame(s)")
    for at, frame in enumerate(meta.animation):
        imgui.push_id(at)
        imgui.set_next_item_width(sp(90))
        changed, duration = controls.input_int("ms", int(frame.duration_ms))
        if changed:
            frames = list(meta.animation)
            frames[at] = TileFrame(local_id=frame.local_id, duration_ms=max(1, duration))
            tab.doc.set_tile_meta(
                index, local, dataclasses.replace(meta, animation=tuple(frames))
            )
        imgui.same_line()
        widgets.muted(f"tile {frame.local_id}")
        imgui.same_line()
        if widgets.icon_button(f"x##tsdel{at}", "Remove this frame", borderless=True):
            frames = [f for pos, f in enumerate(meta.animation) if pos != at]
            tab.doc.set_tile_meta(
                index, local, dataclasses.replace(meta, animation=tuple(frames))
            )
        imgui.pop_id()
    if controls.button(f"Add this tile as a frame##tsaddf{local}", (-1, 0)):
        tab.doc.set_tile_meta(
            ...
            dataclasses.replace(
                meta,
                animation=(*meta.animation, TileFrame(local_id=local, duration_ms=100)),
            ),
        )
    widgets.muted_wrapped(
