"""Build mode's tool column: what to add, how to transform it, and snapping.
...
from .. import build_mode, build_state, icons, widgets
...
def _tool_grid(state: Any) -> None:
    ...
    for index, (key, label, shortcut) in enumerate(build_state.TOOLS):
        ...
        if imgui.button(f"{icon}##buildtool{key}", (width, 30)):
            state.tool = key
        if selected:
...
def _add(ctx: Any, state: Any, doc: Any) -> None:
    ...
    for index, name in enumerate(sorted(bp.GENERATORS)):
        ...
        if imgui.button(f"{icon}##add{name}", (width, 28)):
...
def _object_actions(ctx: Any, state: Any, doc: Any) -> None:
    ...
    if widgets.disabled_button(f"{icons.COPY} Duplicate", has):
        build_mode._duplicate_selection(ctx, state, doc)
    imgui.same_line()
...
def _display(ctx: Any, state: Any) -> None:
    ...
    if changed:
        ...
        view = getattr(ctx, "build_view", None)
        if view is not None:
