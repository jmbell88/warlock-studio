"""``.wblk`` -- the Build document on disk, as a zip.
...
from .document import BuildDoc, Obj, reserve_uid
...
def _material_json(material: gltf.Material) -> dict[str, Any]:
    ...
    Every texture slot is deliberately dropped. Build mode paints none, they
...
def scene_json(doc: BuildDoc) -> str:
...
def wblk_bytes(doc: BuildDoc) -> bytes:
...
def _read_mesh(zf: zipfile.ZipFile, uid: int) -> bm.Mesh:
    ...
    try:
        ...
        raise ValueError(
            f"this build document names an object whose mesh is missing ({name})"
        ) from exc
    ...
    with np.load(io.BytesIO(raw), allow_pickle=False) as npz:
        try:
            ...
            raise ValueError(f"a mesh in this build document is incomplete: {exc}") from exc
    mesh = bm.Mesh(**arrays)
...
def read_wblk(data: bytes) -> BuildDoc:
    """A ``.wblk``'s bytes back into a :class:`BuildDoc`.
    ...
    try:
        ...
        raise ValueError("this is not a Warlock build document") from exc

    version = int(scene.get("version", 0))
    if version > VERSION:
        raise ValueError(
            f"this build document was written by a newer version of Warlock "
