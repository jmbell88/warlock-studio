"""Build mode's controller: opening, saving, exporting, guarding and keys.

Everything here is *about* documents rather than geometry -- the engine under
``build/`` has no idea a job or a task thread exists, and this is the layer that
...
is why saving is a *state* (``BuildTab.saving``) rather than a function call
that returns.
...
Every task key carries the ``build-`` prefix, because the app claims results by
...
from . import build_state, dialogs
from .build_state import BuildState, BuildTab

log = logging.getLogger(__name__)

WBLK_FILTER = ["Warlock build document (*.wblk)", "*.wblk"]


def ensure(ctx: Any) -> BuildState:
    """The mode's state, built on first use.

    Lazy because a session that never opens Build mode should not pay for it,
...
def persist(ctx: Any) -> None:
    ...
    if state is not None:
        ctx.settings.set("build", {"recent": state.recent})


def active(ctx: Any) -> BuildTab | None:
    state = ctx.state.build
...
def adopt(ctx: Any, doc: Any, *, path: Path | None = None, title: str | None = None) -> BuildTab:
    state = ensure(ctx)
    tab = BuildTab(
        doc=doc,
        title=title or build_state.title_for(path),
        path=path,
...
def new_document(ctx: Any) -> BuildTab:
    from .build import document as bd

    return adopt(ctx, bd.BuildDoc(), title="Untitled")


def _load(path: Path) -> dict[str, Any]:
    """Blocking; task thread only. Raises rather than returning a broken tab."""
    from .build import serialize

    doc = serialize.read_wblk(Path(path).read_bytes())
    return {"doc": doc, "path": str(path), "title": build_state.title_for(Path(path))}
...
def ask_open(ctx: Any) -> None:
    ...
    def run() -> dict[str, Any] | None:
        path = dialogs.open_file("Open build document", WBLK_FILTER)
        return None if path is None else _load(path)

    ctx.submit("build-open", run)
...
def open_path(ctx: Any, path: Path) -> None:
    ...
    if existing is not None:
        ...
        return
    ctx.submit(f"build-open:{abs(hash(str(path)))}", _load, path)


# --- saving -----------------------------------------------------------------


def _start(ctx: Any, tab: BuildTab, key: str, run: Any) -> None:
    tab.saving = True
...
def save_to(ctx: Any, tab: BuildTab, path: Path) -> None:
    ...
    _start(ctx, tab, f"build-save:{tab.uid}", run)


def save(ctx: Any, tab: BuildTab | None = None) -> None:
...
def save_as(ctx: Any, tab: BuildTab | None = None) -> None:
    ...
    def run() -> dict[str, Any] | None:
        path = dialogs.save_file(
            "Save build document", f"{title}{build_state.WBLK_SUFFIX}", WBLK_FILTER
        )
        if path is None:
            return None
        path = path.with_suffix(build_state.WBLK_SUFFIX)
        path.write_bytes(data)
        return {"rev": rev, "path": str(path), "retitle": True}

    _start(ctx, tab, f"build-saveas:{tab.uid}", run)


# --- export -----------------------------------------------------------------


def export_asset(ctx: Any, tab: BuildTab | None = None) -> None:
    """Mint an ordinary asset from the document: the point of Build mode.
    ...
    paths learning that Build mode exists.
    ...
    def run() -> dict[str, Any]:
        ...
        job_id = result["id"]
        svc_files.save_build_source(ctx.svc, job_id, wblk)
        return {"job_id": job_id, "exported": True}

    _start(ctx, tab, f"build-export:{tab.uid}", run)
...
def on_task_done(ctx: Any, done: Any) -> None:
    """Called from the app for every ``build-`` key."""
    state = ensure(ctx)
    ...
    if result.get("retitle") and result.get("path"):
        ...
        tab.title = build_state.title_for(tab.path)
...
def guard(ctx: Any, verb: str, proceed: Any) -> bool:
    ...
    is not, because Build is a mode rather than a takeover and its tabs are
...
def handle_key(ctx: Any, event: Any) -> bool:
    """Build mode's shortcuts. -> whether the key was consumed.

    **False with nothing open**, which the caller's fall-through depends on:
    Build mode owns a viewport, and with no document the viewport's own
    shortcuts must still work. With a document open the key is consumed
    unconditionally, because falling through would let it act on a pane Build
    mode has replaced.
    """
    import pygame
...
def _ctrl_key(
    ctx: Any, state: BuildState, tab: BuildTab, doc: Any, name: str, *, shift: bool
) -> bool:
...
def _duplicate_selection(ctx: Any, state: BuildState, doc: Any) -> None:
    from .build import document as bd
