"""The Build viewport: GL, camera, gizmos and picking over the existing stack.
...
imgui backend may still be holding. :meth:`BuildView.draw` forgets the outgoing
...
class _Composite:
    ...
    is not part of it -- Build mode has no skins, which is also why
...
def _materials_key(doc: Any) -> tuple[int, ...]:
    ...
    ``BuildDoc.set_material`` replaces the entry object, so an edit changes the
