...
#: The symmetries that **compose**, in canonical order. A symmetry is a set of
#: these now rather than one of :data:`SYMMETRY`, because that is what Aseprite
#: offers and what an isometric tile actually needs: its two 45-degree mirrors
#: are toggles beside the axis-aligned pair, so "horizontal and top-left
#: diagonal" is an ordinary thing to ask for and was not expressible.
#:
#: :data:`SYMMETRY` survives untouched as the **legacy spelling**, and every
#: one of its members still means what it always meant -- ``"xy"`` is
#: ``"x+y"``, ``"none"`` is the empty set. Keeping it is what makes this an
#: addition rather than a migration: the engine's parameter is still a string,
#: every stored setting still reads, and the ~40 call sites that pass ``"xy"``
#: are unchanged.
SYMMETRY_AXES = ("x", "y", "diag", "anti", "radial")

#: How a composed symmetry is written down: the axes in :data:`SYMMETRY_AXES`
#: order, joined. A canonical spelling rather than a set, because the value
#: goes into a settings file and through an engine parameter that has always
#: been a string.
SYMMETRY_JOIN = "+"

#: The most reflections one dab is allowed to become. The group generated by
#: all four mirrors is D4 (eight points); with a radial of :data:`MAX_RADIAL`
#: it is D32 (sixty-four). A cap rather than an argument that it cannot be
#: exceeded: this bounds the per-dab loop by construction, and the dab loop is
#: the hottest thing in the editor.
MAX_MIRRORS = 64

#: What each legacy :data:`SYMMETRY` member composes to.
_LEGACY_AXES: dict[str, tuple[str, ...]] = {
    "none": (),
    "": (),
    "x": ("x",),
    "y": ("y",),
...
def axes_of(symmetry: Any) -> tuple[str, ...]:
    """Which symmetries are on, in canonical order. The one reader of the field.

    Takes a legacy mode name (``"xy"``), a composed spelling (``"x+diag"``) or
    any iterable of axis names, and answers a canonical tuple. Unknown names
    are dropped rather than raising: the value comes out of a settings file,
    and a symmetry this build does not have is a preference to ignore, not a
    reason for the editor not to start.

    Exported for the same reason :func:`axis_or_default` is -- the canvas draws
    the *guides* for these and there was a stretch where the guide and the
    engine disagreed, which is a line pointing at the wrong place.
    """
    if symmetry is None:
        return ()
    if isinstance(symmetry, str):
        legacy = _LEGACY_AXES.get(symmetry)
        names = legacy if legacy is not None else symmetry.split(SYMMETRY_JOIN)
    else:
        names = tuple(symmetry)
    wanted = {str(name) for name in names}
    return tuple(axis for axis in SYMMETRY_AXES if axis in wanted)


def compose(axes: Any) -> str:
    """The canonical string for a set of axes. ``"none"`` when there are none.

    ``"none"`` rather than ``""`` so that a written setting reads the same way
    it always has and every legacy comparison against it still holds.
    """
    live = axes_of(axes)
    return SYMMETRY_JOIN.join(live) if live else "none"


def toggled(symmetry: Any, axis: str) -> str:
    """*symmetry* with one axis switched. What each toolbar toggle calls."""

    live = list(axes_of(symmetry))
    if axis in live:
        live.remove(axis)
    elif axis in SYMMETRY_AXES:
        live.append(axis)
    return compose(live)
...
def _mirror(
    ...
    **The mirrors compose, and the composition is a group closure** -- which is
    the whole of the 2026-08-23 change and the reason it is a loop rather than
    a longer chain of ``if``s. Two mirrors applied in turn generate a third
    transform that neither names: horizontal and vertical give the diagonal
    one for free (that *was* the hardcoded ``"xy"`` third point), and
    horizontal with the 45-degree mirror generates all eight of D4. Composing
    by hand would mean writing the multiplication table out; iterating to a
    fixed point is the table, and it is what makes an arbitrary combination
    correct rather than only the six that were enumerated.

    Bounded by :data:`MAX_MIRRORS` rather than by an argument that it cannot
    run away, because this is the hottest loop in the editor.

    Two orderings are deliberate and are what keep the arithmetic identical to
    the enumerated version for every legacy mode. The radial rotations are
    generated **directly at their own angles** rather than by composing one
    rotation *n* times, so a 32-way symmetry does not accumulate 31 rounding
    errors; and the mirrors are applied in :data:`SYMMETRY_AXES` order over a
    snapshot, which reproduces ``"xy"``'s old point order exactly.
    """
    axes = axes_of(symmetry)
    if not axes:
        return [point]
    x, y = point
    ...
    if "radial" in axes:
        ...
        points = [point]
    mirrors = [axis_name for axis_name in axes if axis_name != "radial"]
    if not mirrors:
        return points
    # Rounded, because a reflection composed two ways lands on the same point
    # by a different route and float equality would keep both -- which is a
    # doubled dab at every position on the axis itself.
    seen = {(round(px, 6), round(py, 6)) for px, py in points}
    growing = True
    while growing and len(points) < MAX_MIRRORS:
        growing = False
        for name in mirrors:
            for px, py in list(points):
                ...
                if name == "x":
                    ...
                    # Reflection about the 45-degree line through the axis:
                    # swap the two offsets (and negate both for the
                    # anti-diagonal). Written as offsets from the axis for
                    # ``2a - x``'s reason -- the two agree at the centre and
                    # only this form generalises to a moved axis.
                    hit = (ax + dy, ay + dx)
                else:
                    hit = (ax - dy, ay - dx)
                key = (round(hit[0], 6), round(hit[1], 6))
                if key in seen:
                    continue
                seen.add(key)
                points.append(hit)
                growing = True
                if len(points) >= MAX_MIRRORS:
                    return points
    return points
