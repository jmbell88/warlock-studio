...
from . import controls, fonts, icons, motion, probe, quality, theme, tokens
from . import settings as settings_mod
from . import state as app_state
from .tokens import sp

#: ``artifacts.ARTIFACTS*`` and :func:`artifacts.artifacts_for`, re-exported
#: where every caller already names them. The tables moved to a headless module
#: because ``create_stages`` reads them per frame and must not import imgui.
ARTIFACTS = artifacts_mod.ARTIFACTS
ARTIFACTS_2D = artifacts_mod.ARTIFACTS_2D
ARTIFACTS_TILE = artifacts_mod.ARTIFACTS_TILE
ARTIFACTS_TILESHEET = artifacts_mod.ARTIFACTS_TILESHEET
artifacts_for = artifacts_mod.artifacts_for
...
# per-frame function; ``quality`` imports nothing.
AUDIT_UNINFORMATIVE = quality.AUDIT_UNINFORMATIVE
...
def toggle(
    ...
    draw.add_circle_filled(
        (knob_x, top + track * 0.5), radius, imgui.get_color_u32(theme.rgba(theme.KNOB)), 24
    )
    if label:
        draw.add_text(
