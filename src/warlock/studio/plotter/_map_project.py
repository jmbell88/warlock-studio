...
:mod:`.gid`. What this mixin adds is the *document's* five numbers, which every
one of those calls needs and none of them should have to gather.
...
class ProjectionOps:
    ...
    def _lattice(self: MapDoc) -> tuple[str, int, int, int, int]:
        """The five numbers every :mod:`.project` call takes, in its order."""
        return (self.projection, self.width, self.height, self.tile_w, self.tile_h)
    ...
    def draw_order(self: MapDoc):
        ...
        Not ``*self._lattice()``: this one takes the projection and the two cell
        counts and no tile size at all, so spreading the five would be passing
        two arguments it does not have.
        """
