...
| Item | Status |
|---|---|
| `seam.SEAM_MAX` uncalibrated (`pipelines/seam.py`) | **Closed 2026-08-08.** Calibrated on 72 units — `docs/measurements/2026-08-08-seam-threshold.md`, corpus from `scripts/calibrate_seam.py` and `scripts/calibrate_seam_hard.py`. 2.0 raised two false alarms on tiles that wrap perfectly (large flat cells with thin hard lines break the ratio's denominator); the measured empty band is 2.50–5.52 and the constant is now **3.5**. One checkpoint only (turbo at 4 steps) — a CFG base draws harder edges and should re-run the scripts. |
