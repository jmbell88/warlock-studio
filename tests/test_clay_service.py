...
of those paths learning that Clay exists. What the tests below check is
...
def _glb(mesh=None, translation=(0.0, 0.0, 0.0)) -> bytes:
    """Authored bytes, straight out of the writer Clay uses."""
    doc = bd.ClayDoc()
    doc.add_object(
        bd.Obj(
            uid=bd.new_uid(),
...
def test_the_clay_source_is_never_served(svc) -> None:
    ...
    assert files.CLAY_SOURCE not in files.MEDIA
    assert files.CLAY_SOURCE not in files.LISTED


def test_a_build_source_that_is_not_a_zip_is_refused(svc) -> None:
    out = jobs.import_mesh(svc, _glb())
    with pytest.raises(Invalid):
        files.save_clay_source(svc, out["id"], b"not a zip")


def test_an_oversize_build_source_is_refused(svc) -> None:
    out = jobs.import_mesh(svc, _glb())
    with pytest.raises(TooLarge):
        files.save_clay_source(
            svc, out["id"], b"PK\x03\x04" + b"\x00" * files.MAX_CLAY_SOURCE_BYTES
        )
...
def test_the_sidecar_has_no_staleness_rule(svc) -> None:
    ...
    assert files.clay_source_status(svc, out["id"])["exists"] is True
    assert "fresh" not in files.clay_source_status(svc, out["id"])


def test_reading_back_a_stored_build_source_gives_the_same_bytes(svc) -> None:
    out = jobs.import_mesh(svc, _glb())
    data = io.BytesIO(b"PK\x03\x04zipish").getvalue()
    files.save_clay_source(svc, out["id"], data)
    assert (svc.job_dir(out["id"]) / files.CLAY_SOURCE).read_bytes() == data
