...
def test_fitting_shows_the_whole_document_centred():
    view = PaintView()
    paint_state.fit(view, (100, 50), (200.0, 200.0))
...
def test_a_round_trip_through_the_view_is_the_identity():
    ...
    screen = paint_state.to_screen(view, (10.0, 20.0), 4.0, 6.0)
    assert paint_state.to_image(view, (10.0, 20.0), *screen) == pytest.approx((4.0, 6.0))
...
def test_image_coordinates_stay_fractional_for_the_brush():
    ...
    x, y = paint_state.to_image(view, (0.0, 0.0), 3.0, 5.0)
...
def test_zooming_keeps_the_pixel_under_the_cursor_under_the_cursor():
    ...
    before = paint_state.to_image(view, origin, *mouse)
    paint_state.zoom_about(view, origin, mouse, 3.0)
    assert paint_state.to_image(view, origin, *mouse) == pytest.approx(before)
    assert view.zoom > 1.0


def test_zoom_is_clamped_at_both_ends():
    view = PaintView(zoom=1.0)
    paint_state.zoom_about(view, (0.0, 0.0), (0.0, 0.0), 200.0)
    assert view.zoom == pytest.approx(paint_state.MAX_ZOOM)
    paint_state.zoom_about(view, (0.0, 0.0), (0.0, 0.0), -400.0)
    assert view.zoom == pytest.approx(paint_state.MIN_ZOOM)


def test_centring_sets_an_exact_zoom():
    view = PaintView()
    paint_state.centre(view, (100, 100), (400.0, 400.0), 1.0)
...
def test_a_swatch_is_added_once_and_the_row_is_bounded():
    ...
    for i in range(paint_state.MAX_SWATCHES + 5):
        state.add_swatch((i, 0, 0, 255))
    assert len(state.swatches) == paint_state.MAX_SWATCHES
...
def test_every_tool_has_a_shortcut_and_a_label():
    keys = [key for key, _, _ in paint_state.TOOLS]
    assert len(set(keys)) == len(keys)
    assert all(label and shortcut for _, label, shortcut in paint_state.TOOLS)
...
def test_the_tool_groups_do_not_overlap():
    groups = (paint_state.PAINT_TOOLS, paint_state.SHAPE_TOOLS, paint_state.SELECT_TOOLS)
...
def test_recent_files_are_most_recent_first_deduplicated_and_bounded():
    state = PaintState()
    for i in range(paint_state.MAX_RECENT + 5):
        state.remember(Path(f"/tmp/f{i}.ora"))
    assert len(state.recent) == paint_state.MAX_RECENT
...
def test_the_tool_shortcuts_and_the_tool_list_agree():
    ...
    tools = {key for key, _, _ in paint_state.TOOLS}
    assert set(paint_mode.TOOL_KEYS.values()) == tools
    assert len(set(paint_mode.TOOL_KEYS)) == len(paint_mode.TOOL_KEYS)
