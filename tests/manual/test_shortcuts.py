...
CHAPTER = Path(__file__).resolve().parents[2] / "docs" / "manual" / "16-shortcuts.md"
...
def test_no_binding_in_the_sheet_is_missing_from_the_chapter(title):
    ...
    assert not missing, (
        ...
        f"docs/manual/16-shortcuts.md does not"
    )
...
def test_the_installation_chapter_puts_the_health_badge_where_it_is():
    ...
    text = (CHAPTER.parent / "17-installation.md").read_text(encoding="utf-8")
    assert "top bar" not in text.lower()
