"""The winged archetype: the bake, the skeleton it fits, and the clips.

``tests/characters/test_humanoid.py``'s claims, made again for the body plan a
dragon lives on: every shipped species is a shape you can pose, it stays one at
...
#: 1e-5 of unit height is forty microns on a four-metre dragon -- far below
...
def shipped() -> dict[str, tuple[gltf.Model, dict[str, np.ndarray]]]:
    ...
    for key in SILHOUETTES:
        ...
        # ``tusked`` and this archetype's own keys were one collision from picking
...
def _pulled_in(positions: np.ndarray, points: np.ndarray) -> np.ndarray:
    ...
    The two talon landmarks sit **on** the surface by construction and not by
    accident: each is the centre of a sole box's bottom face, which is exactly
    the floor of the bounding box. That is the boundary, where a crossing count
    is a coin toss, so the question the test means -- "is this joint in the
    body" -- is asked one nudge inside.
    """
...
def test_every_baked_mesh_is_one_closed_solid(silhouette, rebuilt):
    ...
    The hardest case in this archetype is the wing: a vane seventeen thousandths
    of a body height thick, crossed by a leading-edge chain and, on the membrane
    groups, by four finger spars. That is why the chain is *thicker* than the
    plate rather than flush with it -- two surfaces that cross transversally
    leave no sliver for the weld to pinch.
    """
...
@pytest.mark.parametrize("silhouette", SILHOUETTES)
def test_a_winged_creature_is_wider_than_it_is_tall(silhouette, rebuilt):
    """The one claim that makes this archetype not a biped with decorations: the
    wings are the widest thing about it. A group whose span did not clear its own
    height would be a creature the ``bird`` flap cycle beats the air with and
    never leaves the ground on."""
    lo, hi = rebuilt[silhouette].bounds
    assert (hi[0] - lo[0]) > 1.15 * (hi[1] - lo[1]), silhouette
...
def test_the_regions_on_a_mesh_are_the_ones_its_features_call_for(silhouette, rebuilt):
    ...
    key that looks configured and does nothing. Every winged group carries all
    seven, which is a stronger claim than it looks: ``horn`` is the talons even
    on the two groups with no horns, and ``beak`` is the snout tip on the two
    groups with no beak, because one material is what each pair actually is."""
    baked = rebuilt[silhouette]
...
@pytest.mark.parametrize("silhouette", SILHOUETTES)
def test_the_generated_mesh_fits_the_bird_template_exactly(silhouette, rebuilt):
...
def test_a_channel_at_either_bound_keeps_the_skeleton_in_the_body(silhouette, channel, rebuilt):
    ...
    ``wingspan`` is the reason the field is proportional to x rather than a
    push outward, and the reason the test names both bounds: a constant push at
    -1 moves a wing joint further than its own distance from the midline, which
    leaves the two wings crossed over each other and their bones in the air.
    """
    baked = rebuilt[silhouette]
...
def test_every_shipped_species_is_a_body_its_own_skeleton_sits_inside(species, rebuilt):
    ...
    six of them stack at once. The dragon carries +0.6 bulk with +0.55 span,
    +0.45 neck and +0.55 tail together, and no single-channel test would catch
    the pair that only fails in combination."""
...
def test_the_species_are_visibly_different_flyers():
    """The point of parameterising rather than modelling six meshes is that the
    parameters actually separate the species. A bat is nearly all wing, a raven
    is nearly all bird, and a dragon and a griffin sit between them -- measured
    off the displaced vertices as span over height. If this ever collapses, the
    registry is six palettes."""
    ...
    assert width["raven"] < width["griffin"] < width["dragon"] < width["bat"]
    assert length["bat"] < length["bird"] < length["dragon"]
    # Every one of them flies: the span clears the height, always.
    assert min(width.values()) > 1.0


# --- the clips --------------------------------------------------------------


def test_the_winged_archetype_uses_the_shipped_template_and_clip_library():
    """No ``dragon.json`` and no ``clips/dragon.json``. A dragon is a species of
    the ``bird`` body plan, membrane and all: what separates it from a raven is
    a mesh the generator grows over the same three wing bones, and a second
    skeleton for those bones would be two skeletons that drift."""
    arch = familylib.get_archetype(ARCHETYPE)
    assert (arch.template, arch.clip_library) == ("bird", "bird")
    assert "dragon" not in rigging.templates()
    library = rigging.clip_library("bird")
    assert {c["name"] for c in library["clips"]} >= {"idle", "walk", "run", "attack", "jump"}
    assert library["space"] == "delta"


def test_every_animation_a_winged_recipe_asks_for_expands_to_its_frames():
    ...
    recipe = Recipe.from_dict(
        {"family": "dragon", "animations": {"idle": 4, "walk": 8, "run": 8, "attack": 6, "jump": 6}}
    )
    layout = charsheet.resolve_layout(recipe.layout_payload())
    records = clipslib.expand_clips("bird", layout)
...
def test_every_species_instantiates_grounded_at_its_own_height(species, tmp_path):
    ...
    assert inst.bone_names == [b["name"] for b in rigging.get_template("bird").bones]
