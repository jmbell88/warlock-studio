"""The amorphous archetype: the bake, the skeleton it fits, and the clips.

``tests/characters/test_humanoid.py``'s claims, made again for a body plan with
no limbs at all: every shipped species is a shape you can pose, it stays one at
both ends of every appearance slider, and the shipped clips move it somewhere
worth rendering. What a blob needs a skeleton for is a centre of mass that can
rise and sink and a surface that can bulge, so the claims below are about the
column and the four lobes rather than about arms.
...
#: 1e-5 of unit height is eighteen microns on a 1.8 m elemental -- far below
#: what a 64px sprite can show and far above float noise.
TOL = 1e-5
...
def shipped() -> dict[str, tuple[gltf.Model, dict[str, np.ndarray]]]:
    ...
    for key in SILHOUETTES:
        ...
        # ``tusked`` and the quadruped's tusker were one letter from picking
...
def _pulled_in(positions: np.ndarray, points: np.ndarray) -> np.ndarray:
    ...
    ``base``'s head is the centre of the base cylinder's flat bottom, which is
    the floor of the box, and ``top``'s tail is the body ellipsoid's own pole,
    which is its ceiling. Both are exactly on the boundary, where a crossing
    count is a coin toss, so the question the test means -- "is this joint in
    the body" -- is asked one nudge inside.
    """
...
def test_every_baked_mesh_is_one_closed_solid(silhouette, rebuilt):
    ...
    The interesting case in this archetype is the crystal group, where six
    shards cut out through one ellipsoid at six different angles and the
    intersections are the only places two surfaces here meet at all.
    """
...
@pytest.mark.parametrize("silhouette", SILHOUETTES)
def test_a_blob_has_no_front(silhouette, rebuilt):
    """The claim that makes this archetype amorphous rather than a creature
    without legs: in plan view it is round, so no yaw of the sheet camera is a
    privileged one. A group that grew a front would be a group whose eight-yaw
    sheet showed eight different characters."""
    lo, hi = rebuilt[silhouette].bounds
    ratio = float((hi[0] - lo[0]) / (hi[2] - lo[2]))
    assert 0.75 < ratio < 1.35, f"{silhouette}: width/depth {ratio:.2f}"
...
def test_the_regions_on_a_mesh_are_the_ones_its_features_call_for(silhouette, rebuilt):
    ...
    key that looks configured and does nothing.

    ``core`` is the nucleus, and this is where the translucency decision shows
    up as a measurement rather than a paragraph: nothing in the render path
    reads alpha, so the nucleus is *on the outside*. On the crystal group that
    is six shards spread up and down the body; everywhere else it is the cap the
    body thins to. The two rules produce visibly different distributions, and
    the assertion is that they do.
    """
    baked = rebuilt[silhouette]
    names = familylib.get_archetype(ARCHETYPE).regions
    present = {names[i] for i in np.unique(baked.regions)}
    assert present == set(names), silhouette

    from warlock.characters.amorphous.generate import _centroids

    centroids = _centroids(baked.positions, baked.loops, baked.starts)
    core = centroids[baked.regions == names.index("core")]
    if generate.GROUPS[silhouette].shards:
        assert core[:, 1].min() < 0.5, "the shards should not all be a cap"
        assert float(core[:, 1].max() - core[:, 1].min()) > 0.30
    else:
        assert core[:, 1].min() > 0.60, "the cap should not reach the equator"
...
@pytest.mark.parametrize("silhouette", SILHOUETTES)
def test_the_generated_mesh_fits_the_blob_template_exactly(silhouette, rebuilt):
...
def test_a_channel_at_either_bound_keeps_the_skeleton_in_the_body(silhouette, channel, rebuilt):
    ...
    ``ripple`` is the reason the surface field is three sines and not the hash
    the region rules use: a hash is a different number at every vertex, so as a
    *displacement* it does not roughen a surface, it tears one -- and a torn
    surface at either bound is a lobe joint hanging outside a hole.
    """
    baked = rebuilt[silhouette]
...
def test_every_shipped_species_is_a_body_its_own_skeleton_sits_inside(species, rebuilt):
    ...
    five of them stack at once. The ooze carries -0.75 viscosity with +0.6 lobe
    and -0.55 crown together, which is the flattest body this archetype makes,
    and no single-channel test would catch the pair that only fails in
    combination."""
...
def test_the_species_are_visibly_different_shapes():
    """The point of parameterising rather than modelling five meshes is that the
    parameters actually separate the species. An elemental stands up, an ooze
    spreads out, and a slime and a cloud sit between them -- measured off the
    displaced vertices as width over height. If this ever collapses, the
    registry is five palettes."""
    ...
    assert width["elemental"] < width["ghost"] < width["slime"] < width["cloud"]
    assert width["cloud"] < width["ooze"]
    # Round in plan at every species' own defaults, not only at the bake.
    for key in SPECIES:
        assert 0.75 < width[key] / length[key] < 1.35, key


# --- the clips --------------------------------------------------------------


def test_the_amorphous_archetype_uses_the_shipped_template_and_clip_library():
    """No ``slime.json`` and no ``clips/slime.json``. The template is named for
    the shape and not for the genre skin, which is what lets a cloud and a
...
def test_every_animation_an_amorphous_recipe_asks_for_expands_to_its_frames():
    ...
    recipe = Recipe.from_dict(
        {"family": "slime", "animations": {"idle": 4, "walk": 8, "run": 8, "attack": 6, "jump": 6}}
    )
    layout = charsheet.resolve_layout(recipe.layout_payload())
    records = clipslib.expand_clips("blob", layout)
