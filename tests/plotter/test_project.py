...
def test_every_cell_round_trips_from_its_own_centre(projection, shape):
    ...
    for column in range(width):
        for row in range(height):
            x, y = project.cell_origin(
                projection, width, height, tile_w, tile_h, column, row
            )
            back = project.cell_at(
                projection, width, height, tile_w, tile_h, x + tile_w / 2, y + tile_h / 2
            )
...
def test_an_orthogonal_cell_lands_where_it_always_did(shape):
    ...
    for column in range(width):
        for row in range(height):
            assert project.cell_origin(
                project.ORTHOGONAL, width, height, tile_w, tile_h, column, row
            ) == (column * tile_w, row * tile_h)
    assert project.map_size(project.ORTHOGONAL, width, height, tile_w, tile_h) == (
        width * tile_w,
        height * tile_h,
    )
...
def test_an_isometric_map_is_the_bounding_box_of_its_diamonds():
    ...
    size = project.map_size(project.ISOMETRIC, width, height, tile_w, tile_h)
    assert size == ((width + height) * tile_w // 2, (width + height) * tile_h // 2)
    corners = [
        project.cell_corner(project.ISOMETRIC, width, height, tile_w, tile_h, c, r)
...
def test_the_four_cells_meeting_at_a_node_each_own_their_own_side(projection):
    ...
    """
    args = (projection, 5, 5, 32, 16)
    node = project.cell_corner(*args, 2, 2)
    owners = set()
    for column, row in ((1, 1), (2, 1), (1, 2), (2, 2)):
        centre = project.cell_corner(*args, column + 0.5, row + 0.5)
        probe = (
            node[0] + (centre[0] - node[0]) * 0.1,
            node[1] + (centre[1] - node[1]) * 0.1,
        )
        assert project.cell_at(*args, *probe) == (column, row)
...
def test_visible_bounds_take_four_corners_not_two():
    ...
    args = (project.ISOMETRIC, 8, 8, 32, 16)
    size = project.map_size(*args[:1], 8, 8, 32, 16)
    c0, r0, c1, r1 = project.cell_bounds(*args, 0, 0, size[0], size[1])
    assert (c0, r0) == (0, 0)
    assert (c1, r1) == (7, 7)


def test_bounds_are_clamped_to_the_map():
    args = (project.ORTHOGONAL, 4, 3, 16, 16)
    assert project.cell_bounds(*args, -500, -500, 5000, 5000) == (0, 0, 3, 2)


@pytest.mark.parametrize("projection", project.PROJECTIONS)
def test_draw_order_visits_every_cell_once(projection):
    cells = list(project.draw_order(projection, 5, 4))
    assert len(cells) == 20
...
def test_isometric_draw_order_is_back_to_front():
    ...
    depths = [c + r for c, r in project.draw_order(project.ISOMETRIC, 6, 4)]
    assert depths == sorted(depths)


def test_orthogonal_draw_order_is_row_major():
    assert list(project.draw_order(project.ORTHOGONAL, 3, 2)) == [
...
def test_an_object_round_trips_through_tileds_coordinate_space(projection):
    ...
    *height*, not in the projected plane. A position that made the trip
    ...
    args = (projection, 6, 5, 32, 16)
    for point in ((0.0, 0.0), (96.0, 40.0), (17.5, 3.25)):
        stored = project.object_from_pixels(*args, *point)
        back = project.object_to_pixels(*args, *stored)
        assert back == pytest.approx(point)


def test_an_orthogonal_object_is_not_converted_at_all():
    args = (project.ORTHOGONAL, 6, 5, 32, 16)
    assert project.object_to_pixels(*args, 12.5, 7.0) == (12.5, 7.0)
    assert project.object_from_pixels(*args, 12.5, 7.0) == (12.5, 7.0)
