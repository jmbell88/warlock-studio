"""Bake the humanoid archetype's silhouette meshes, and report the numbers.
...
and the ``.glb`` / ``.masks.npz`` pair is the record of *what* --
``tests/characters/test_humanoid.py`` re-runs the first and compares it to the
second, so a kernel that changes its mind fails a test instead of quietly
shipping a different ogre.

The report at the bottom is the argument for baking per **silhouette group**
rather than per species: it prints how many species each group serves and what
one more of each would cost.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    for silhouette in wanted:
        ...
        print(
            f"{silhouette:<9} {stats['faces']:>6} faces  {stats['vertices']:>6} verts  "
