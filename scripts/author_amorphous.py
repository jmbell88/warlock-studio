"""Bake the amorphous archetype's silhouette meshes, and report the numbers.

    uv run python scripts/author_amorphous.py            # measure, write nothing
    uv run python scripts/author_amorphous.py --write    # regenerate the assets

The assets under ``src/warlock/characters/amorphous/`` are **checked in**, for
...
record of *what*; ``tests/characters/test_amorphous.py`` re-runs the first and
