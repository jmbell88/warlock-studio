...
| `embedded atlas tilesets` | round-trips | Map-local atlas definitions and properties are read and modeled without loss. **Written back as an external `.tsx`**, not re-embedded: both exporters emit the portable `tilesets/` bundle for every tileset, so an embedded atlas comes home beside the map rather than inside it. Semantics survive; the embedding does not. Read through the **same element definition** an external `.tsx` uses (`tsx.tileset_from_element`), the way the two JSON spellings already shared `tileset_from_json`, so the pair cannot drift: until 2026-09-03 the embedded path had a copy of that construction which dropped the five presentation fields, the foreign Wang model and the image's `trans` colour key, and `presentation-112.tmx` therefore read `object_alignment` as `unspecified` where its own `.tmj` read `bottomleft`. fixture: `typed-embedded-112`; the drift itself is caught by fixture: `presentation-112` and fixture: `wang-112`, whose two spellings now agree field for field. |
...
| Per-frame palettes | **round-trips** | #20 (retired 2026-08-31) | ~~One table per document; there is only ever one palette to write.~~ A frame's own table is written as a palette chunk on that frame, and only where it differs from **the frame before it** — which is what makes a frame returning to the document's own colours emit a chunk restoring them rather than silently inheriting its neighbour. The document stores whole tables and the format stores deltas applied from their own frame onward, so the deltas are computed back out here; that is a pure function of two adjacent tables. Indexed only, and whole tables rather than the narrowed `first`/`last` range the chunk allows. A document that has never used the feature writes exactly the bytes it always wrote. |
...
| A track’s, a cel’s or a tag’s user data | written, only when set | #14 (partially retired 2026-08-30) | One `0x2020` chunk immediately after the chunk it describes — that adjacency is the format’s only way of saying whose it is, so the layer chunks and their notes are interleaved rather than written in two passes, and a cel’s note follows its own cel or link chunk (a link’s two chunks disagree correctly). Tag notes are a run after the tags chunk, written for **all** tags or none, since the reader counts its way along it. A document with no note writes no chunk at all, so its bytes are the bytes it wrote before the feature existed. The tags chunk’s own three colour bytes stay zero — Aseprite 1.3 does the same and puts the swatch in the user-data chunk, and writing both would be two answers to one question. |
...
| Cel z-index | kept (still documents warned) | #12 (retired 2026-08-30) | The field lands in `Animation.cel_z`, per slot, so a linked pair declaring two different offsets arrives as one shared `Layer` sitting at two heights. ~~"a cel's z-index was dropped; layer order is stacking order."~~ A **one-frame** file opens as a still document (divergence 22) and has no grid for an offset to live in, so there alone it is still said out loud: "a cel's z-index needs a timeline; this file has one frame." |
| User data on a layer, a cel or a tag | kept (still documents warned) | #14 (partially retired 2026-08-30) | The text and the colour land on `Track.note`, `Animation.cel_notes` (per slot, so a linked pair carries two) and `Tag.note`. A user-data chunk names no owner — it describes the chunk before it — so the reader tracks one (`_Parse.ud_owner`). A tag’s pre-1.3 colour bytes in the tags chunk itself are read too, and a following user-data colour overrides them. ~~"user data and timeline colours are not kept; the drawing is"~~. A **one-frame** file opens as a still document (divergence 22), which has no track, no grid slot and no tag: "user data needs a timeline; this file has one frame." |
| A user-data **properties map** (the chunk’s third flag) | warned | #14 (remainder) | "custom properties are not kept; the text and colour are" — a typed key/value tree under extension ids, a document format of its own inside the file. |
| User data on a **slice** | warned, slice kept | #14 (remainder) | "a slice’s user data was dropped; the slice was not" — `Slice` has no note field and adding one would be a fourth model to keep in step. |
| A **tileset’s own** user data | warned, tileset kept | #14 (remainder) | "a tileset’s own user data was dropped; its tiles were not" — the first chunk of the run after a `0x2023`, by the spec’s own convention. |
| Per-tile user data | warned, tiles kept | #14 (remainder) | Its own, narrower sentence ("the tiles are") — the second and later chunks of that run. The picture survives; only the metadata about individual tiles does not. |
| Per-frame palettes | **kept** | #20 (retired 2026-08-31) | ~~"per-frame palettes are not kept; the final table is used."~~ The delta chain is resolved at read time into one whole table per frame, and **frame 0's is the document's** — the honest base, being what a reader ignoring the later chunks would have shown, and identical to the final table for any file whose palette never changes. Only frames that actually differ from it get an override, because an override equal to the default is a difference the user would see in the palette panel and could not explain. Both palette chunk kinds are read this way, the pre-1.0 pair included — which is where per-frame palettes actually live. |
...
| The header's pixel aspect ratio (`pixel width`/`pixel height`) | dropped | — | Non-square pixels are not a thing this editor models: a canvas is a square lattice everywhere in it, and honouring the ratio would mean either resampling on open (a different picture) or carrying an aspect through every view, export and sheet geometry. The two bytes are skipped with the rest of the header's tail. |
| The header's grid rectangle (`grid x/y/width/height`) | dropped | — | Read as part of the skipped header tail. Inker has a grid of its own and it is view state rather than document data, so an imported one would be a setting arriving from a file and silently changing what the next drag snaps to. |
...
| A group's `composite-op` (blend mode on a group as a unit) | **kept** | ~~The document model has no group-level blend: a group is a membership fact, not a compositing stage. A Krita group set to Multiply therefore renders pass-through on load — visibly wrong before anything is saved — and the attribute is gone after.~~ **Retired 2026-08-31.** A group is a compositing stage when it says so: the attribute reads into `GroupNode.blend` through the same `composite.OPS_ORA` map a layer's does, and a Krita group set to Multiply now renders as Multiply. It is written back **only when the mode is not `normal`** — `svg:src-over` is what a reader assumes from an absent attribute, so spelling it out would change the bytes of every file that has ever held a folder and say nothing. A mode this build cannot reproduce reads as `normal`, which is the tolerance rule the layer reader already followed. |
...
| `isolation` | **round-trips** | ~~The writer states the ORA default rather than preserving what it read, for the same reason as `composite-op`: nothing in the model distinguishes an isolated group from a pass-through one.~~ **Retired 2026-08-31.** `GroupNode.isolate` is that distinction, and the attribute is written from it — `isolate` or `auto`, whichever the group actually is. A group carrying a blend mode writes `isolate` whatever the flag says, because `groups.isolated` is one rule and a mode implies it; a file claiming otherwise would render differently from the app that wrote it. |
