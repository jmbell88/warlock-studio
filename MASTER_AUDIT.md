...
- **Current-tree spot check (2026-08-11, post-audit).** The working tree has moved past `de87838` (uncommitted bench/metrics work unrelated to either audit). Re-verified that day: `pyproject.toml:3` still `0.0.22`, `__version__` still `0.0.21`, CHANGELOG top entry still `0.0.21`, `docs/TODO.md` still absent. The headline findings were not stale.
  > **Correction (2026-08-12).** That spot check is historical. `pyproject.toml` was reverted to `0.0.21` under D1 and `scripts/preflight.py` now gates the lockstep, so the version surfaces agree; `docs/TODO.md` stays deliberately absent per D2. See "Outcomes (2026-08-12)" below for the authoritative state.
